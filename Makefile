...
test-short: ## Run unit tests only (skip e2e/stress tagged !short)
	$(CGO) $(GO) test -short -tags=short -count=1 -timeout=5m ./...

test: ## Run all tests including e2e/stress (needs FUSE + TORBOX_API_KEY)
	$(CGO) $(GO) test -count=1 -timeout=15m ./...

test-race: ## Run unit tests with race detector
	$(CGO) $(GO) test -race -short -tags=short -count=1 -timeout=10m ./...

test-coverage: ## Run unit tests with coverage report
	$(CGO) $(GO) test -short -tags=short -cover -coverprofile=coverage.out -timeout=5m ./...
